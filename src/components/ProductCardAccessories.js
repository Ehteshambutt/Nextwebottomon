...
const ProductCardAccessories = ({ name, price, image, hoverImage, discount, size, category, selectedGrid, index, pageType ,CallingFrom}) => {
    ...
    console.log(CallingFrom,'CallingFrom___')
      useEffect(() => {
          const fetchMattresses = async () => {
              try {
                  const response = await axios.get('http://localhost:5000/api/v1/Accessories');
                  console.log('API Response:', response.data);
                  if(CallingFrom==='Duvetts'){
                        const orthopaedicMattresses = response.data.filter(mattress => mattress.Type === 'Duvetts');
                        setMattresses(orthopaedicMattresses)}
                        else  if(CallingFrom==='Pillows'){
                                const orthopaedicMattresses = response.data.filter(mattress => mattress.Type === 'Pillows');
                                setMattresses(orthopaedicMattresses);
                              }else  if(CallingFrom==='MattressToppers'){
                                const orthopaedicMattresses = response.data.filter(mattress => mattress.Type === 'Mattress Toppers');
                                setMattresses(orthopaedicMattresses);
    ...
    return (
        ...
        selectedGrid === 0 ?
         <div key={index} className={`flex p-[20px] md:gap-x-4  border border-primary ${index >= 0 && index < 9 ? 'border-b-0' : ''} `}>
